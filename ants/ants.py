...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        location_num = 0
        while location != hive:
            if random_or_none(location.bees) != None and self.min_range <= location_num < self.max_range:
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        """Checks if HungryAnt is digesting"""
        # BEGIN Problem 6
        "*** YOUR CODE HERE ***"
        if self.digesting:
            self.digesting -= 1
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt does nothing each turn. Has a large armor value"""
        ...
        if self.is_imposter == True:
            ...
            while location:
                if location.ant != None:
                    if location.ant.is_doubled == False:
                        location.ant.damage *= 2
                        location.ant.is_doubled = True
                    if location.ant.is_container:
                        if location.ant.contained_ant and location.ant.contained_ant.is_doubled == False:
