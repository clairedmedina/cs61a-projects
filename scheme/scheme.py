...
def do_and_form(expressions, env):
    ...
    if scheme_falsep(value):
        return False
    next = do_and_form(expressions.second, env)
    if next is True:
        return value
    return next
    # END PROBLEM 13
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    "*** YOUR CODE HERE ***"
    formals, evals = nil, nil
    while bindings is not nil:
        bind = bindings.first
        formals = Pair(bind.first, formals)
        bindings = bindings.second
        evals = Pair(scheme_eval(bind.second.first, env), evals)
        check_form(bind, 2, 2)
    check_formals(formals)
    return env.make_child_frame(formals, evals)
    # END PROBLEM 15
