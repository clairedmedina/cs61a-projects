...
def group_by_centroid(restaurants, centroids):
    ...
    # BEGIN Question 4
    "*** YOUR CODE HERE ***"
    x = [[find_closest(restaurant_location(i), centroids), i] for i in restaurants] #this adds the closest centroid to the 0-index
...
def find_predictor(user, restaurants, feature_fn):
    ...
    #sum(zip([i - mean(xs) for i in xs] * [j - mean(ys) for j in ys]))
    b = S_xy / S_xx
    a = mean(ys) - b * mean(xs)
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    "*** YOUR CODE HERE ***"
    return max([find_predictor(user, reviewed, i)[0] for i in feature_fns], key=lambda a: find_predictor(user, reviewed, a)[1])
    # END Question 8
...
def rate_all(user, restaurants, feature_fns):
    ...
    # BEGIN Question 9
    "*** YOUR CODE HERE ***"
    rest_names = [restaurant_name(i) for i in restaurants]
    predictor_scores = [predictor(i) for i in restaurants]
    dict_all = dict(zip(rest_names, predictor_scores))
    for i in reviewed:
        if restaurant_name(i) in dict_all.keys():
            dict_all[restaurant_name(i)] = user_rating(user, restaurant_name(i))
    return dict_all
    # END Question 9
